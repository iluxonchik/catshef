...
class SnackBar(object):
    ...
    request.session[SnackBar.SESSION_KEY] stores a list of expected data 
    objects.
    ...
    def get(cls, request):
        ...
        """
        if (not cls.__is_initialized(request) 
            or not request.session[SnackBar.SESSION_KEY]):
            return SnackBar.from_data(None)
        
        cls.__check_session_type(request)
        data_list = request.session[SnackBar.SESSION_KEY]
        cls.clear(request)
        return SnackBar.from_data(data=data_list)
